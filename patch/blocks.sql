...
-- DEV 1: CREATION OF TABLE 'BLOCKS'
...
-- Check if constraint exists before adding (using simple ALTER for simplicity)
...
-- Add BlockID if it doesn't exist (Ignoring error if it does for safety in this script type)
-- Note: In pure SQL scripts without procedures, we typically run ALTER. 
-- If you get an error that BlockID exists, just ignore it.
ALTER TABLE Users ADD COLUMN BlockID VARCHAR(10); 
...
ALTER TABLE `Groups` ADD COLUMN BlockID VARCHAR(10); 
...
-- DEV 3: ADD THE SEEDS & DATA
-- 1. Insert missing roles for Chairperson and Adviser
INSERT INTO Roles (RoleID, RoleName) VALUES 
(5, 'Chairperson'),
(6, 'Adviser');
-- 2. INSERT COURSES 
-- Now that the column exists, we can insert the course definitions.
UPDATE Courses SET CourseCode = 'CMPE312', CourseName = 'Methods of Research' WHERE CourseID = 'C1';
UPDATE Courses SET CourseCode = 'CMPE404', CourseName = 'CPE Practice and Design 1' WHERE CourseID = 'C2';
UPDATE Courses SET CourseCode = 'CMPE407', CourseName = 'CPE Practice and Design 2' WHERE CourseID = 'C3';

-- 3. PRE-REQUISITE: Insert the Advisers (Users) FIRST
...
-- 5. Insert Faculty Details (For Pure Faculty Only)
...
-- 6. Insert the Section/Block definitions
...
-- 7. Update existing Students (101-112) to Block 3-1
...
-- 8. Update existing Groups (G1-G3) to Block 3-1 (Fix for Error 1062)
-- We rename GroupCode to '3101' etc. so '3301' is free for the new groups.
UPDATE `Groups` SET BlockID = '2025-31', AdviserUserID = 122, GroupCode = '3101' WHERE GroupID = 'G1';
UPDATE `Groups` SET BlockID = '2025-31', AdviserUserID = 122, GroupCode = '3102' WHERE GroupID = 'G2';
UPDATE `Groups` SET BlockID = '2025-31', AdviserUserID = 122, GroupCode = '3103' WHERE GroupID = 'G3';
...
-- 10. Insert New Students (Users 201-260)
...
-- 11. Insert Group Members (Defining Leaders and Members)
